...
# #VEHICLE
DRIVE_LOOP_HZ = 70 #20      # the vehicle loop will pause if faster than this speed.
...
ROI_CROP_TOP = 50       #35                    #the number of rows of pixels to ignore on the top of the image
ROI_CROP_BOTTOM = 70    #10             #the number of rows of pixels to ignore on the bottom of the image
...
STEERING_LEFT_PWM = 505         #pwm value for full left steering
STEERING_RIGHT_PWM = 275        #pwm value for full right steering

#THROTTLE
#THROTTLE_CHANNEL = 1            #channel on the 9685 pwm board 0-15
THROTTLE_FORWARD_PWM = 500      #pwm value for max forward throttle, default: 435
...
JOYSTICK_MAX_THROTTLE = .75         # [0,1], this scalar is multiplied with the throttle value (-1 to 1) to limit the maximum throttle. This can help if you drop the controller or just don't need the full speed available.
...
AI_MIN_THROTTLE = 0.3    #0.1      # the throttle lower bound during AI mode, in case the model outputs a too small values leading the car to stop
